...
#
# Usage: ios/build-local-device.sh [AppleTeamID] [/path/to/ultima4]
...
DEBUG_TOOLS="${ZU4_IOS_DEBUG_TOOLS:-ON}"
...
cmake "$ZU4_SRC" -G Xcode \
  ...
  -DZU4_IOS_BUILD_NUMBER="$BUILD_NUMBER" -DZU4_IOS_DEBUG_TOOLS="$DEBUG_TOOLS" \
...
xcodebuild -project zu4.xcodeproj -target zu4 -configuration Release \
  -sdk iphoneos -arch arm64 -jobs "${ZU4_BUILD_JOBS:-2}" -allowProvisioningUpdates DEVELOPMENT_TEAM="$TEAM"
