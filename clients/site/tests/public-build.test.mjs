...
test("public routes include the homepage, real client and matching source, not test hooks or game sprites",async()=>{
  ...
  assert.match(await readFile(new URL("play/session-orchestrator.js",root),"utf8"),/UltimatumSessionOrchestrator/);
  assert.match(await readFile(new URL("play/storage-provider-contract.js",root),"utf8"),/assertStorageProvider/);
  ...
  assert.ok(source.files.some(file=>file.path.endsWith("packages/catalog/schemas/catalog-entry.schema.json")));
  assert.ok(source.files.some(file=>file.path.endsWith("packages/library/schemas/library-record.schema.json")));
  ...
  assert.ok(source.files.some(file=>file.path.endsWith("packages/session-orchestrator/schemas/session-progress.schema.json")));
  assert.ok(source.files.some(file=>file.path.endsWith("packages/import-framework/README.md")));
  assert.ok(source.files.some(file=>file.path.endsWith("packages/import-framework/schemas/install-plan.schema.json")));
