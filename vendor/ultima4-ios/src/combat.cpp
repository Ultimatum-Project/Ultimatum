...
#ifdef ZU4_IOS
        // The native HUD already identifies the active fighter and weapon.
        if (settings.fastCombatPresentation) screenMessage("\020");
        else
#endif
    ...
    if (!quick || player->isDisabled()){
        ...
        do {
            ...
            if (focus >= c->party->size()) {
                ...
                gameUpdateScreen();
#ifdef ZU4_IOS
                EventHandler::sleep(MobileCombat::roundPauseMilliseconds(settings.fastCombatPresentation));
#else
 ...
 */
bool CombatController::attack() {
#if defined(ZU4_IOS) || defined(ZU4_WEB)
    PartyMember *attacker = getCurrentPlayer();
    const weapon_t *weapon = attacker->getWeapon();
    MobileCombatTarget selected;
    if (mobileSelectedTarget(&selected)) {
        int selectedRange = (weapon->flags & WEAP_CHOOSEDISTANCE)
            ? selected.distance : weapon->range;
        return performAttack(selected.direction, selectedRange);
    }
    // A prepared repeat must never become a blind directional attack if its
    // enemy or weapon ceased to be valid before the confirmation was handled.
    if (mobileRepeatWeapon) {
        mobileClearTarget();
        screenMessage("Repeat unavailable. Choose a target.\n");
        return false;
    }
#endif
#ifdef ZU4_IOS
...
#ifdef ZU4_IOS
        range = gameGetAttackRange(weapon->range);
    ...
    return performAttack(dir, range);
}

bool CombatController::performAttack(Direction dir, int range) {
    PartyMember *attacker = getCurrentPlayer();
    const weapon_t *weapon = attacker->getWeapon();
#if defined(ZU4_IOS) || defined(ZU4_WEB)
    mobileLastAttacks[focus].clear();
#endif
...
#if defined(ZU4_IOS) || defined(ZU4_WEB)
        Creature *candidate = map->creatureAt(*i);
        uint64_t identity = candidate ? candidate->mobileCombatIdentity.value : 0;
#endif
        if (attackAt(*i, attacker, MASK_DIR(dir), range, distance)) {
#if defined(ZU4_IOS) || defined(ZU4_WEB)
            mobileLastAttacks[focus] = {identity, weapon};
#endif
            foundTarget = true;
...
#if defined(ZU4_IOS) || defined(ZU4_WEB)
std::vector<MobileCombatTarget> CombatController::mobileAttackTargets() {
    std::vector<MobileCombatTarget> targets;
    PartyMember *attacker = getCurrentPlayer();
    if (!attacker || attacker->isDisabled()) return targets;
    const weapon_t *weapon = attacker->getWeapon();
    const Direction directions[] = {DIR_NORTH, DIR_EAST, DIR_SOUTH, DIR_WEST};
    for (Direction direction : directions) {
        std::vector<Coords> path = gameGetDirectionalActionPath(MASK_DIR(direction), MASK_DIR_ALL,
            attacker->getCoords(), 1, weapon->range,
            weapon->flags & WEAP_ATTACKTHROUGHOBJECTS ? NULL : &Tile::canAttackOverTile, false);
        int distance = 1;
        for (const Coords &coords : path) {
            Creature *candidate = map->creatureAt(coords);
            bool selectableDistance = (weapon->flags & WEAP_CHOOSEDISTANCE) != 0;
            bool rightFixedDistance = !(weapon->flags & WEAP_ABSOLUTERANGE) ||
                                      distance == weapon->range;
            if (candidate && (selectableDistance || rightFixedDistance)) {
                targets.push_back({coords, direction, distance, candidate});
                break;
            }
            ++distance;
        }
    }
    std::stable_sort(targets.begin(), targets.end(), [](const MobileCombatTarget &a,
                                                        const MobileCombatTarget &b) {
...
bool CombatController::mobileSelectedTarget(MobileCombatTarget *target) {
    if (!mobileTargetSelected) return false;
    PartyMember *attacker = getCurrentPlayer();
    if (!attacker || (mobileRepeatWeapon && attacker->getWeapon() != mobileRepeatWeapon))
        return false;
    std::vector<MobileCombatTarget> targets = mobileAttackTargets();
    for (const MobileCombatTarget &candidate : targets) {
        if (zu4_coords_equal(candidate.coords, mobileTargetCoords) &&
            candidate.creature->mobileCombatIdentity.value == mobileTargetIdentity) {
            if (target) *target = candidate;
            return true;
        }
    }
    // Preserve the repeat guard until Attack or Clear handles an invalidated
    // preparation. HUD queries must not turn it into a directional attack.
    mobileTargetSelected = false;
    return false;
}

bool CombatController::mobileSelectTarget(const Coords &coords) {
    std::vector<MobileCombatTarget> targets = mobileAttackTargets();
    for (const MobileCombatTarget &candidate : targets) {
        if (!zu4_coords_equal(candidate.coords, coords)) continue;
        if (mobileTargetSelected && zu4_coords_equal(mobileTargetCoords, coords)) {
            mobileClearTarget();
            return true;
        }
        mobileTargetCoords = coords;
        mobileTargetIdentity = candidate.creature->mobileCombatIdentity.value;
        mobileRepeatWeapon = nullptr;
...
bool CombatController::mobileCycleTarget(int step) {
    std::vector<MobileCombatTarget> targets = mobileAttackTargets();
    if (targets.empty()) {
        mobileClearTarget();
        return false;
    ...
    int current = -1;
    if (mobileTargetSelected) {
        for (std::size_t i = 0; i < targets.size(); ++i)
            if (zu4_coords_equal(targets[i].coords, mobileTargetCoords)) {
                current = (int)i;
                break;
            }
    }
    int count = (int)targets.size();
    int next = current < 0 ? (step < 0 ? count - 1 : 0)
                           : (current + (step < 0 ? -1 : 1) + count) % count;
    mobileTargetCoords = targets[next].coords;
    mobileTargetIdentity = targets[next].creature->mobileCombatIdentity.value;
    mobileRepeatWeapon = nullptr;
    mobileTargetSelected = true;
    return true;
}

void CombatController::mobileClearTarget() {
    mobileTargetSelected = false;
    mobileTargetIdentity = 0;
    mobileRepeatWeapon = nullptr;
}

bool CombatController::mobileLastAttackTarget(MobileCombatTarget *target) {
    PartyMember *attacker = getCurrentPlayer();
    if (!attacker || attacker->isDisabled() || focus >= AREA_PLAYERS) return false;
    for (const MobileCombatTarget &candidate : mobileAttackTargets()) {
        if (mobileLastAttacks[focus].matches(candidate.creature->mobileCombatIdentity.value,
                                            attacker->getWeapon())) {
            if (target) *target = candidate;
            return true;
        }
...
bool CombatController::mobilePrepareLastAttack() {
    MobileCombatTarget target;
    if (!mobileLastAttackTarget(&target)) return false;
    mobileTargetCoords = target.coords;
    mobileTargetIdentity = target.creature->mobileCombatIdentity.value;
    mobileRepeatWeapon = getCurrentPlayer()->getWeapon();
    mobileTargetSelected = true;
    return true;
}
#endif
