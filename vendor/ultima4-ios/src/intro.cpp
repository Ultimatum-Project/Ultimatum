...
static bool mobileIntroMenuShowing = false;
...
bool IntroController::init() {
	...
	bool skipInitialTitles = bSkipTitles;
#ifdef ZU4_IOS
	// The animated title remains available from the native menu. On touch,
	// start at actionable choices rather than requiring an unexplained Return.
	skipInitialTitles = true;
	mobileIntroMenuShowing = false;
#elif defined(ZU4_WEB)
    // The HTML title owns New Game / Continue; creation keeps the original story.
    skipInitialTitles = true;
#endif
    ...
    // Draw the menu background before the first native panel appears. The
    // mobile path also skips titles, even when bSkipTitles was not supplied.
    if (skipInitialTitles)
        updateScreen();
...
bool IntroController::keyPressed(int key) {
    ...
    case INTRO_MENU:
        ...
        case 'i':
#ifdef ZU4_IOS
            if (!gameChooseSaveSlotForNewGame()) break;
#endif
            errorMessage.erase();
            initiateNewGame();
            break;
...
#ifdef ZU4_IOS
        {
            SaveSlotSelectionResult selection = gameChooseSaveSlotForJourney();
            if (selection == SAVE_SLOT_SELECTION_NEW_GAME) {
                if (!gameChooseSaveSlotForNewGame()) break;
                errorMessage.erase();
                initiateNewGame();
                break;
            }
            if (selection != SAVE_SLOT_SELECTION_SELECTED) break;
        }
#endif
            journeyOnward();
            break;
...
#ifdef ZU4_IOS
    // The mobile intro jumps directly from the startup progress display to
    // this menu. Its two legacy background slices do not cover the narrow gap
    // between them, so clear the complete framebuffer before composing them.
...
#ifdef ZU4_IOS
    std::string nameBuffer = gameGetCharacterName();
#elif defined(ZU4_WEB)
    std::string nameBuffer = webReadPrompt("text", "Name your Avatar",
        "By what name shalt thou be known in this world and time? (12 characters)",
        {{"\033", "Cancel creation"}}, 12, true);
    if (nameBuffer.find_first_not_of(' ') == std::string::npos) nameBuffer.clear();
#else
...
#ifdef ZU4_WEB
    std::string answer = webReadPrompt("choice", "Your Avatar", "Choose your character",
        webVendorOptions("mf", {"Male", "Female"}, false, true));
    int sexChoice = answer.empty() ? '\033' : answer[0];
#else
    int sexChoice = gameGetVendorChoice("mf", "Choose your character", false, {"Male", "Female"});
#endif
    if (sexChoice != 'm' && sexChoice != 'f') {
        menuArea.disableCursor();
        screenEnableCursor();
        updateScreen();
        return;
...
void IntroController::finishInitiateGame(const std::string &nameBuffer, SexType sex)
{
    ...
    std::string saveDirectory = gameBeginSaveDirectory();
    if (saveDirectory.empty()) { errorMessage = "Unable to prepare save!"; updateScreen(); return; }
    ...
    bool saved = saveGameWrite(&saveGame, saveGameFile) != 0;
    if (fclose(saveGameFile) != 0) saved = false;
    if (!saved) { errorMessage = "Unable to write adventure!"; updateScreen(); return; }
    ...
    if (!saveGameFile) { errorMessage = "Unable to write creatures!"; updateScreen(); return; }
    saved = saveGameMonstersWrite(NULL, saveGameFile) != 0;
    if (fclose(saveGameFile) != 0) saved = false;
#ifdef ZU4_IOS
    // Publish the empty journal with the new party, never clear the old one first.
    if (saved) saved = TopicJournal().save(saveDirectory + "topics.txt");
    // Snapshot publication requires all adventure-owned metadata. A new
    // character begins with a valid, entirely unrevealed exploration map.
    if (saved) saved = gameSaveEmptyExplorationMap(saveDirectory);
#endif
    if (!saved || !gamePublishSaveDirectory(saveDirectory, false)) {
        errorMessage = "Unable to finish adventure save!"; updateScreen(); return;
    }
...
#if defined(ZU4_IOS) || defined(ZU4_WEB)
    gameGetVendorChoice("", introGypsy[GYP_SEGUE1], true, {}, true, false, true);
    showText((std::string)introGypsy[GYP_SEGUE2]);
    gameGetVendorChoice("", introGypsy[GYP_SEGUE2], true, {}, true, false, true);
#else
...
#if defined(ZU4_IOS) || defined(ZU4_WEB)
        gameGetVendorChoice("", "The story begins — " + std::to_string(storyInd + 1) +
            "/24\n\n" + introText[storyInd], true, {}, true, false, true);
#else
        eventHandler->pushController(&pauseController);
...
void IntroController::startQuestions() {
    ...
    while (1) {
        ...
        // Present the lead-up with a touch continuation on iOS.
#if defined(ZU4_IOS) || defined(ZU4_WEB)
        gameGetVendorChoice("", std::string(introGypsy[questionRound == 0 ? GYP_PLACES_FIRST :
            (questionRound == 6 ? GYP_PLACES_LAST : GYP_PLACES_TWOMORE)]) + "\n" + introGypsy[GYP_UPON_TABLE] +
            "\n" + introGypsy[questionTree[questionRound * 2] + 4] + " and " +
            introGypsy[questionTree[questionRound * 2 + 1] + 4] + ". Consider this:",
        ...
        do {
#ifdef ZU4_WEB
            std::string answer = webReadPrompt("choice", "The virtues",
                getQuestion(questionTree[questionRound * 2], questionTree[questionRound * 2 + 1]),
                webVendorOptions("ab", {"Choose A", "Choose B"}, false, false));
            choice = answer.empty() ? '\033' : answer[0];
#else
            choice = gameGetVendorChoice("ab", getQuestion(questionTree[questionRound * 2],
                questionTree[questionRound * 2 + 1]), false, {"Choose A", "Choose B"}, false,
                false, true);
#endif
...
void IntroController::journeyOnward() {
    if (!gamePrepareJourney()) { updateScreen(); return; }
    FILE *saveGameFile;
    ...
    if (saveGameFile) {
        ...
        bool readComplete = saveGameRead(&saveGame, saveGameFile) != 0;
        if (readComplete && SaveValidation::characters(saveGame))
            validSave = true;
        //delete saveGame;
        fclose(saveGameFile);
    }
...
#ifdef ZU4_IOS
    if (mode == INTRO_MENU && eventHandler->getController() == this && !mobileIntroMenuShowing) {
        mobileIntroMenuShowing = true;
        int choice = gameGetVendorChoice("jisra", "Ultima IV\n\nContinue your adventure, begin anew, or sign in to keep your saves available across devices.", false,
            {"Journey onward", "New game", "Sign in or create account", "Watch introduction", "About"}, false, true);
        if (choice == 's') {
            zu4_mobile_account_from_title();
        } else if (choice == 'a') {
            gameGetVendorChoice("", "About Ultima IV\n\nThis mobile port uses the open-source zu4 engine. Ultima IV was created by Richard Garriott and Origin Systems.\n\nzu4 is free software under the GNU GPL version 2 or later.", true);
        } else {
            keyPressed(choice);
        }
        // Journey Onward and a completed New Game mark the intro controller
        // done. Keep the guard raised until it is destroyed so no second menu
        // can be presented over the newly initialized world.
        if (!EventHandler::getControllerDone())
            mobileIntroMenuShowing = false;
        return;
    }
#endif
