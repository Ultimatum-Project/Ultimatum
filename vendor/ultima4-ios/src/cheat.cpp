...
bool CheatMenuController::keyPressed(int key) {
    ...
    case 's':
        ...
        screenMessage("What?\n");
        cheatSummonCreature(gameGetInput());
        break;
...
bool cheatCreateTransport(char transport, Direction direction) {
    if (!c || !c->location || !c->location->map->isWorldMap() || direction == DIR_NONE)
        return false;
    ...
    MapTile *choice = transport == 's' ? &ship : (transport == 'h' ? &horse :
        (transport == 'b' ? &balloon : NULL));
    if (!choice) return false;
    Coords coords = c->location->coords;
    movedir(&coords, direction, c->location->map);
    if (zu4_coords_equal(coords, c->location->coords)) return false;
    MapTile *ground = c->location->map->tileAt(coords, WITHOUT_OBJECTS);
    bool validGround = transport == 's' ? ground->getTileType()->isSailable()
                                        : ground->getTileType()->isWalkable();
