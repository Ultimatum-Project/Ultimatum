...
test("public routes include the homepage, real client and matching source, not test hooks or game sprites",async()=>{
  ...
  assert.match(play,/web-overlay-host.js/);
  ...
  assert.match(await readFile(new URL("play/web-overlay-host.js",root),"utf8"),/UltimatumWebOverlayHost/);
  ...
  assert.equal(JSON.parse(await readFile(new URL("play/overlays.manifest.json",root))).overlaysVersion,1);
  ...
  assert.ok(source.files.some(file=>file.path.endsWith("packages/ui/schemas/overlay-manifest.schema.json")));
  ...
  assert.ok(source.files.some(file=>file.path.endsWith("ports/ultima-iv/overlays.manifest.json")));
