...
#ifdef ZU4_IOS
    // Native conversation panels scroll and size text for the device. Keep
    // utterances intact instead of imposing the DOS text window's page size.
    return std::list<std::string>{text};
#else
    std::string str = text;
    int pos, real_lines;
...
std::list<std::string> Person::getConversationText(Conversation *cnv, const char *inquiry) {
    ...
    if (isVendor()) {
        ...
        if (cnv->state == Conversation::INTRO) {
            ...
            script->clearDisplayedText();
            script->run("intro");

            while (script->getState() != Script::STATE_DONE) {
                // Gather input for the script
                if (script->getState() == Script::STATE_INPUT) {
                    gameObserveVendorDialogue(script->getDisplayedText());
                    switch(script->getInputType()) {
                    case Script::INPUT_CHOICE: {
                        const std::string &choices = script->getChoices();
                        // Get choice
                        std::vector<std::string> names;
                        for (char choice : choices) names.push_back(script->getChoiceName(choice));
                        char val = gameGetVendorChoice(choices, script->getDisplayedText(), false, names);
                    ...
                    case Script::INPUT_KEYPRESS:
                        gameGetVendorChoice(" \015\033", script->getDisplayedText(), true);
                        break;

                    case Script::INPUT_NUMBER: {
                        int val = gameGetAmountInput(script->getInputMaxLen(), script->getDisplayedText());
                        if (val < 0) {
                            // Native Cancel is not an offer to pay zero. Leave
                            // without running the pending transaction branch.
                            script->setState(Script::STATE_DONE);
                        } else script->setVar(script->getInputName(), val);
                    } break;

                    case Script::INPUT_STRING: {
                        std::string str = gameGetVendorTopic(script->getDisplayedText(), script->getTextChoices(), script->getInputMaxLen());
                        if (str.size()) {
...
#ifdef ZU4_IOS
                        int player = gameGetPlayer(true, false, "Choose a companion for this service");
#elif defined(ZU4_WEB)
                        std::vector<WebPromptOption> companions;
                        for (int i = 0; i < c->party->size(); ++i) {
                            PartyMember *member = c->party->member(i);
                            companions.push_back({std::to_string(i + 1), member->getName() + " — " +
                                std::to_string(member->getHp()) + "/" + std::to_string(member->getMaxHp()) + " HP"});
                        }
                        companions.push_back({"\033", "Cancel"});
                        std::string answer = webReadPrompt("player", "Choose a companion", script->getDisplayedText(), companions);
                        int player = answer.empty() ? -1 : atoi(answer.c_str()) - 1;
#else
                    ...
                    // A cancelled amount terminates the transaction rather
                    // than submitting a zero-valued offer.
                    if (script->getState() == Script::STATE_DONE) break;
        ...
        // Preserve the final displayed reply as well as input-step dialogue.
        gameObserveVendorDialogue(script->getDisplayedText());
        // Unload the script
        script->unload();
