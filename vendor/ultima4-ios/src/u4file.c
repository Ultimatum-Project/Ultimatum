...
static char game_data_path[1024];

void zu4_set_game_data_path(const char *path) {
	if (!path || !path[0]) return;
	snprintf(game_data_path, sizeof(game_data_path), "%s", path);
	zippath = game_data_path;
	dospath = game_data_path;
}
...
U4FILE *u4fopen(const char *fname) {
	...
	/* These three VGA images intentionally retain .ega names. Prefer the
	 * overlay only in VGA mode; never replace originals or executable data. */
	bool ambiguousRune = !strcmp(fname, "rune_6.ega") || !strcmp(fname, "rune_7.ega") || !strcmp(fname, "rune_8.ega");
	bool upgradeFirst = settings.videoType == 1 && ambiguousRune;
	u4f = u4fopen_zip(fname, upgradeFirst ? &u4upgrade : &u4base);
	if (u4f) {
	...
	u4f = ambiguousRune && settings.videoType == 0 ? NULL : u4fopen_zip(fname, upgradeFirst ? &u4base : &u4upgrade);
	if (u4f) {
		zu4_file_close = &zu4_file_zip_close;
		zu4_file_seek = &zu4_file_zip_seek;
		zu4_file_tell = &zu4_file_zip_tell;
...
void u4find_path(char *path, size_t psize, const char *fname, const char *subpath) {
	...
	if (f == NULL) {
		if (subpath && subpath[0] == '/')
			snprintf(path, psize, "%s/%s", subpath, fname);
		else
