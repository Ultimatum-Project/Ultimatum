...
static void handleKeyDownEvent(const SDL_Event &event, Controller *controller, updateScreenCallback updateScreen) {
    ...
    /* A held hardware/controller direction may keep moving, but must never
     * repeatedly reopen a door or restart a conversation. */
#ifdef ZU4_IOS
    bool movementKey = event.key.keysym.sym == SDLK_UP || event.key.keysym.sym == SDLK_DOWN ||
        event.key.keysym.sym == SDLK_LEFT || event.key.keysym.sym == SDLK_RIGHT;
    if (movementKey) zu4_mobile_set_bump_input_eligible(event.key.repeat == 0);
#endif
    processed = controller->notifyKeyPressed(key);
#ifdef ZU4_IOS
    if (movementKey) zu4_mobile_set_bump_input_eligible(1);
#endif
...
#ifdef ZU4_IOS
            if (handleMobileLifecycle(event)) continue;
            if (mobileInputBlocked(event)) continue;
            if ((zu4_ios_native_text_input_active() || zu4_journal_panel_is_visible()) &&
                (event.type == SDL_KEYDOWN || event.type == SDL_KEYUP)) continue;
#endif
			switch (event.type) {
			default:
				break;
...
void EventHandler::run() {
    ...
    while (!ended && !controllerDone) {
        ...
        while (SDL_PollEvent(&event)) {
			...
			case SDL_USEREVENT:
			#ifdef ZU4_WEB
				if (event.user.code == ZU4_WEB_ACTION_CONTEXT ||
				    event.user.code == ZU4_WEB_ACTION_ADJACENT ||
				    event.user.code == ZU4_WEB_ACTION_PROMPT ||
                    event.user.code == ZU4_WEB_ACTION_TEST ||
                    event.user.code == ZU4_WEB_ACTION_MENU ||
                    event.user.code == ZU4_WEB_ACTION_WALK_START ||
                    event.user.code == ZU4_WEB_ACTION_WALK_STEP ||
                    event.user.code == ZU4_WEB_ACTION_CHECKPOINT ||
                    (event.user.code > ZU4_WEB_ACTION_GAMEPLAY_BASE &&
                     event.user.code <= ZU4_WEB_ACTION_GAMEPLAY_END)) {
					zu4_web_dispatch_action(event.user.code,
					    static_cast<int>(reinterpret_cast<intptr_t>(event.user.data1)));
					break;
				}
			#endif
			#ifdef ZU4_IOS
				if (event.user.code == ZU4_IOS_ACTION_EVENT) {
					zu4_mobile_perform_action((int)(intptr_t)event.user.data1,
					                          (int)(intptr_t)event.user.data2);
					break;
				}
			#endif
				eventHandler->getTimer()->tick();
		...
		zu4_ogl_swap();
#ifdef ZU4_WEB
        // The native clients own their blocking event loops. Asyncify yields
        // the browser build here so rendering, input, and timers can progress.
        emscripten_sleep(1);
#endif
	}
}
