...
void useStone(int item) {
        ...
        if (needStoneNames) {
            ...
            if (c->location->context & CTX_ALTAR_ROOM) {
                ...
                if (attr && needStoneNames) {
                    screenMessage("\n%c:", 'E'-needStoneNames);
                    itemHandleStones(gameGetStoneInput().c_str());
                }
     ...
     */
        ...
        std::string question = virtueMask > 0 ?
            "What virtue dost stem from " + std::string(getBaseVirtueName(virtueMask)) + "?" :
            "What virtue exists independently of Truth, Love, and Courage?";
        std::string virtue = gameGetVirtueInput(question.c_str());
 ...
 */
std::vector<const ItemLocation *> itemOwnedUsable() {
    std::vector<const ItemLocation *> result;
    for (const auto &item : items) {
        if (!item.shortname || !item.useItem || !item.isItemInInventory) continue;
        if (!strcmp(item.shortname, "keys") || !strcmp(item.shortname, "stones")) continue;
        if (item.data == ITEM_SKULL && item.useItem == useSkull &&
            (c->saveGame->items & ITEM_SKULL_DESTROYED)) continue;
        if (item.isItemInInventory(item.data)) result.push_back(&item);
    }
    return result;
}
