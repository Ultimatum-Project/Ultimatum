...
void ImageMgr::reloadForSettings(SettingsData *newSettings) {
    /* Tile images are owned separately and must be released by the caller
     * first. Preserve only the live renderer surface shared by every set. */
    for (std::map<std::string, ImageSet *>::iterator i = imageSets.begin(); i != imageSets.end(); ++i) {
        ImageSet *set = i->second;
        for (std::map<std::string, ImageInfo *>::iterator j = set->info.begin(); j != set->info.end(); ++j) {
            ImageInfo *info = j->second;
            if (info->name != "screen" && info->image != NULL) {
                zu4_img_free(info->image);
                info->image = NULL;
            }
        }
