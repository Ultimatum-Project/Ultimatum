...
void zu4_ogl_swap() {
    ...
    int active = zu4_mobile_world_status(status, sizeof(status));
    if (zu4_ios_world_rect(&destination, width, height, active ? status : NULL)) {
        SDL_Rect world = {8, 8, 176, 176};
        SDL_RenderSetLogicalSize(renderer, 0, 0);
        SDL_RenderClear(renderer);
        SDL_RenderCopy(renderer, texture, &world, &destination);
    } else if (!active && zu4_ios_title_rect(&destination, width, height)) {
        SDL_RenderSetLogicalSize(renderer, 0, 0);
        SDL_RenderClear(renderer);
