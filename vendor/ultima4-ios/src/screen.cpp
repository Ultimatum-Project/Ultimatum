...
bool screenApplyVideoType(int videoType) {
    if ((videoType != 0 && videoType != 1) || (videoType == 1 && !u4isUpgradeAvailable()))
        return false;
    Tileset::unloadAllImages();
    settings.videoType = videoType;
    imageMgr->reloadForSettings(&settings);
    charsetInfo = NULL;
    gemTilesInfo = NULL;
    ...
    if (game && c && c->location && c->location->map && c->stats) {
        game->mapArea.reinit();
        game->mapArea.setTileset(c->location->map->tileset);
        DungeonViewer.reinit();
        game->initScreen();
        ImageInfo *borders = imageMgr->get(BKGD_BORDERS);
        if (!borders || !borders->image)
            return false;
        zu4_img_draw(borders->image, 0, 0);
        c->stats->update();
        gameUpdateScreen();
    }
    return true;
...
void screenMessage(const char *fmt, ...) {
    ...
    // screenMessage can recurse while wrapping its desktop text area.
    static int messageDepth = 0;
    struct MessageScope { int &depth; MessageScope(int &d): depth(d) { ++depth; } ~MessageScope() { --depth; } } scope(messageDepth);
    if (messageDepth == 1) {
#ifdef ZU4_IOS
        zu4_ios_message(buffer);
#endif
#ifdef ZU4_WEB
        zu4_web_message(buffer);
#endif
    }
#endif
    screenHideCursor();
