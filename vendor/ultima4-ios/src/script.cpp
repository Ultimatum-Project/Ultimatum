        ...
        if (xmlNodeIsText(current)) {
            ...
            if (output)
                *output += content;
                ...
                if (displayedText.size() > 8192) displayedText.erase(0, displayedText.size() - 8192);
            }
...
std::vector<std::string> Script::getTextChoices() {
    std::vector<std::string> result;
    for (xmlNodePtr scope = scriptNode; scope; scope = scope->parent) {
        for (xmlNodePtr item = scope->children; item; item = item->next) {
            if (target == (const char *)item->name && xmlPropExists(item, idPropName.c_str()))
                result.push_back(xmlGetPropAsString(item, idPropName.c_str()));
        }
        if (xmlStrEqual(scope->name, (const xmlChar *)"script")) break;
    }
    return result;
}
std::string Script::getChoiceName(char choice) {
    xmlNodePtr item = find(scriptNode, nounName, std::string(1, choice));
    return item && xmlPropExists(item, "name") ? xmlGetPropAsString(item, "name") : "";
}
