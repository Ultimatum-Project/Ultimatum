...
#if defined(ZU4_IOS) || defined(ZU4_WEB)
    std::vector<MobileCombatTarget> mobileAttackTargets();
    bool mobileSelectTarget(const Coords &coords);
    bool mobileCycleTarget(int step);
    bool mobileSelectedTarget(MobileCombatTarget *target);
    void mobileClearTarget();
    bool mobileLastAttackTarget(MobileCombatTarget *target);
    bool mobilePrepareLastAttack();
    bool mobileHasTargetPreparation() const { return mobileTargetSelected || mobileRepeatWeapon; }
#endif
...
private:
    bool performAttack(Direction dir, int range);
#if defined(ZU4_IOS) || defined(ZU4_WEB)
    bool mobileTargetSelected = false;
    Coords mobileTargetCoords = {0, 0, 0};
    uint64_t mobileTargetIdentity = 0;
    const weapon_t *mobileRepeatWeapon = nullptr;
    MobileCombat::LastAttack mobileLastAttacks[AREA_PLAYERS];
#endif
