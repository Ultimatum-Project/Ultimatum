...
void codexStart() {
     ...
     */
    codexHandleWOP(gameGetRememberedAnswer("What is the Word of Passage?"));
}
...
void codexHandleWOP(const std::string &word) {
    ...
    if (strcasecmp(word.c_str(), "veramocor") == 0) {
        ...
        codexHandleVirtues(gameGetVirtueInput(codexVirtueQuestions[0], true));
...
void codexHandleVirtues(const std::string &virtue) {
    ...
    if ((current < VIRT_MAX) &&
        ...
        codexHandleVirtues(gameGetVirtueInput(codexVirtueQuestions[current], true));
    }
...
bool codexHandleInfinityAnyKey(int key, void *data) {
    ...
    codexHandleInfinity(gameGetRememberedAnswer("What is the one thing which encompasses and is the whole of all undeniable Truth, unending Love, and unyielding Courage?"));
    return true;
}
